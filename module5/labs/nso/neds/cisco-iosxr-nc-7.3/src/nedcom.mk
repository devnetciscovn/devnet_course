...
YANG_YANG   := $(sort $(wildcard yang/*.yang))
YANG        := $(filter-out cliparser-extensions-%.yang,$(YANG_YANG:yang/%.yang=%.yang))
ifeq ($(PACKAGE-TYPE),netconf)
# All files in single bundle
 YANG_NETCONF ?= $(YANG)
else
# Separate bundles
...
ifneq ($(PACKAGE-TYPE),netconf)
	...
	$(YPP) --out-dir=tmp-yang $(foreach v,$(YPP_VARS),--var $(v)=$($(v))) 'yang/*.yang'
	$(YPP) --from='\n//#.*' --to='' 'tmp-yang/*.yang'
.PHONY: mkdirs
...
../package-meta-data.xml: package-meta-data.xml.in
	...
	elif [ ! -z "$(YANG_ID_MODULE)" -a -f $(YANG_ID_MODULE) ]; then \
	    cp $< $@; \
...
tmp-yang/fxs-config: $(call y,$(YANG_CONFIG))
	...
	ln -s $(call pp,$(YANG_CONFIG)) $(call ppp,$(YANG_ID_LEGACY)) tmp-yang/config
...
# Netsim
netsim:
#	Use all cores but one, but with an upper limit. Will create
#	havoc in the Erlang runtime otherwise.
	$(SAY) MAKE netsim
